...
Panel {
  ...
  function launchAgent(agentId) {
    var id = agentId || (root.provider ? root.provider.providerId : "")
    if (id === "claude") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "claude", "--permission-mode", "auto"])
    } else if (id === "codex") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "codex", "--approve-for-me"])
    } else if (id === "cursor") {
      Quickshell.execDetached(["cursor"])
    } else if (id === "hermes") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "hermes"])
    } else if (id === "devin") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "devin"])
    } else if (id === "groq") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "groq"])
    } else if (id === "openrouter" || id === "opencode") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "opencode"])
    } else if (id === "agy") {
      Quickshell.execDetached(["omarchy-launch-tui", "--app-id=org.omarchy.agent", "agy"])
    } else if (id === "a0") {
  ...
  function loginAgent(agentId) {
    var id = agentId || (root.provider ? root.provider.providerId : "")
    if (id === "claude") {
      Quickshell.execDetached(["omarchy-launch-tui", "claude", "auth", "login"])
    } else if (id === "codex") {
      Quickshell.execDetached(["omarchy-launch-tui", "codex", "login"])
    } else if (id === "cursor") {
      Quickshell.execDetached(["xdg-open", "https://www.cursor.com/settings"])
    } else if (id === "devin") {
      Quickshell.execDetached(["xdg-open", "https://app.devin.ai/settings/api-keys"])
    } else if (id === "openrouter" || id === "factory") {
      Quickshell.execDetached(["xdg-open", "https://openrouter.ai/keys"])
    } else if (id === "fireworks") {
      Quickshell.execDetached(["xdg-open", "https://fireworks.ai/account/api-keys"])
    } else {
      root.refreshNow()
    }
    root.close()
  ...
  BarIconButton {
    ...
    onPressed: function(buttonCode) {
      var now = Date.now()
      var dt = now - root.lastPressMs
      root.lastPressMs = now
      if (buttonCode !== Qt.LeftButton) {
        singleClickTimer.stop()
        root.clickCount = 0
        if (buttonCode === Qt.RightButton) root.launchAgent("openrouter")
        else if (buttonCode === Qt.MiddleButton) root.selectProvider(root.providerIndex + 1)
        return
      }
      if (dt < 300 && root.clickCount === 1) {
        singleClickTimer.stop()
        root.clickCount = 0
        root.launchAgent("openrouter")
      } else {
        root.clickCount = 1
      ...
      Flickable {
        ...
        Column {
          ...
          PanelHero {
            ...
            trailingControl: Component {
              Button {
                ...
                onClicked: root.launchAgent(root.provider ? root.provider.providerId : "")
              }
            }
          }
          ...
          Grid {
            ...
            width: parent.width
            columns: Math.min(3, Math.max(2, Math.ceil(root.providers.length / 2)))
              ...
              Button {
                ...
                onClicked: {
                  ...
                  if (currentTime - lastClickTime < 400) {
                    root.launchAgent(modelData.providerId)
                  }
          ...
          // ---------- Status & Login Action Card ----------
          BorderSurface {
            visible: !!root.provider && (String(root.provider.usageStatusText || "") !== "" || !root.provider.ready)
            width: parent.width
            implicitHeight: statusCol.implicitHeight + Style.space(20)
            color: root.alpha(root.urgent, 0.12)
            borderSpec: Border.flat(root.alpha(root.urgent, 0.40), 1)
            ...
            Column {
              ...
              Button {
                ...
                fontSize: Style.font.bodySmall
                verticalPadding: Style.space(6)
                onClicked: root.loginAgent(root.provider ? root.provider.providerId : "")
              }
            }
          }
