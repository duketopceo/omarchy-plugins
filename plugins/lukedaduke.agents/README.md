...
`omarchy plugin disable lukedaduke.agents`.
...
- IPC: `omarchy-shell lukedaduke.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/omarchy/shell.json`. The
top-level keys can be set with
`omarchy bar set lukedaduke.agents <key> <value>`:
...
```bash
omarchy bar set lukedaduke.agents refreshIntervalSec 300 --json
omarchy bar set lukedaduke.agents syncDir '~/Sync/agent-usage'
```
...
```bash
omarchy bar set lukedaduke.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
