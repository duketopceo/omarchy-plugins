...
Item {
  ...
  property var enabledProviders: {
    ...
    var order = { claude: 1, devin: 2, agy: 3 }
    result.sort(function(a, b) {
      var ao = order[a.providerId] || 100
      var bo = order[b.providerId] || 100
  ...
  function displayProvider(record) {
    ...
    return {
      providerId: id,
      providerName: id === "openrouter" ? "opencode" : (id === "agy" ? "antigravity" : String(record.name || record.id)),
